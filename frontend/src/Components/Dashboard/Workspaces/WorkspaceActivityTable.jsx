...
import { WorkspaceActivityContext } from "./WorkspaceActivityList";

const WorkspaceActivityTable = () => {
    const [searchQuery, setSearchQuery] = useState('');
    const dashboardContext = useContext(DashboardContext);
    const workspaceActivityContext = useContext(WorkspaceActivityContext);
    ...
    const activities = workspaceActivityContext.activities || [];
    const filteredActivity = activities.filter(activity => {
        const activityIdMatch = activity.workspaceActivityId.toString().toLowerCase().includes(searchQuery);
        const workspaceIdMatch = activity.workspaceId.toString().toLowerCase().includes(searchQuery);
        ...
        const lastNameMatch = activity.userLastName.toString().toLowerCase().includes(searchQuery);
        ...
        return activityIdMatch || workspaceIdMatch || userIdMatch || userNameMatch || lastNameMatch || actionTypeMatch || entityNameMatch || ActionDateMatch
    });
    return (
                        ...
                        {filteredActivity.length > 0 ? (
                            filteredActivity.map((activity, index) => (
                                <tr key={index} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                    <td className="px-6 py-4">{activity.workspaceActivityId}</td>
                                    <td className="px-6 py-4">{activity.workspaceId}</td>
...
export default WorkspaceActivityTable;